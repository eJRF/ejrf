from questionnaire.models import Question, QuestionGroupOrder, QuestionOption, QuestionGroup, SubSection, Section, \
    Questionnaire
...
class QuestionGroupUtilTest(BaseTest):
    def setUp(self):
        ...
        self.order1 = QuestionGroupOrder.objects.create(question=self.question1, question_group=self.question_group,
                                                        order=1)
        self.order2 = QuestionGroupOrder.objects.create(question=self.question2, question_group=self.question_group,
                                                        order=2)
        self.order3 = QuestionGroupOrder.objects.create(question=self.question3, question_group=self.question_group,
                                                        order=3)
        self.order4 = QuestionGroupOrder.objects.create(question=self.question4, question_group=self.question_group,
                                                        order=4)
    ...
    def test_maps_question_type_with_orders(self):
        expected_map = {self.question4.answer_type: [{'option': self.option1, 'order': self.order4}],
                        self.question1.answer_type: [{'option': self.option1, 'order': self.order1}], 'Number': [],
                        'Text': []}
    ...
    def test_maps_multiple_questions_type_with_orders_when_they_are_of_same_type(self):
        question5 = Question.objects.create(text='question 5', instructions="instruction 5", UID='04444',
                                            answer_type='Date')
        ...
        expected_map = {self.question4.answer_type: [{'option': self.option1, 'order': self.order4},
                                                     {'option': self.option1, 'order': order5}],
                        self.question1.answer_type: [{'option': self.option1, 'order': self.order1}], 'Number': [],
                        'Text': []}
