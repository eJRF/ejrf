...
class QuestionGroupOrderTest(BaseTest):
    ...
    def test_store(self):
        question_group_order = QuestionGroupOrder.objects.create(question_group=self.grouped_question,
                                                                 question=self.question, order=1)
    ...
    def test_order_knows_it_is_last_of_his_type_in_group(self):
        ...
        question_group_order = QuestionGroupOrder.objects.create(question_group=self.grouped_question,
                                                                 question=self.question, order=1)
        another_question_group_order = QuestionGroupOrder.objects.create(question_group=self.grouped_question,
                                                                         question=last_text_question, order=2)
        number_group_order = QuestionGroupOrder.objects.create(question_group=self.grouped_question,
                                                               question=number_question, order=3)
